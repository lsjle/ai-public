...
def replace_letters_with_numbers(input_list):
    ...
    return result_list
```
This code can categorize the letter we will be used as index in the future.
# 12/28
...
```
We use United State Code aka USC as our train data. The dataset is not provided to public, however it can be found online easily.
# 1/4
...
We use ssh to access the AI server at our school and use ChatGLM-6b as our base model and fine-tune the model with ptunning, which is both fast and effective. It's all about debugging and waiting.

The code can be found on the original creator's [repository](https://github.com/THUDM/ChatGLM3). For security reason, the process of debugging is not shown publicly.
# 1/11
